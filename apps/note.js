...
export class ji_note extends plugin {
  ...
  async noteResin(e) {
    ...
    let Resin = Number(
      e.msg.replace(/#?(原神|星铁)?体力推送(阈值)?\s*/i, "").trim()
    );
    let max = e.msg.includes("星铁") ? 120 : 100;
    let g = e.msg.includes("星铁") ? "sr_Resin" : "gs_Resin";
    if (Resin < max)
      return e.reply(
        `\n${e.msg.includes("星铁") ? "星铁" : "原神"}体力推送阈值不可小于${
          e.msg.includes("星铁") ? "【120】" : "【100】"
        }`,
        false,
        { at: true }
      );
    ...
    return await e.reply(
      `\n${e.msg.includes("星铁") ? "星铁" : "原神"}体力推送阈值【${Resin}】`,
      false,
      { at: true }
    );
  }
  ...
  async noteconfig(e) {
    ...
    let uid = Number(
      e.msg.replace(/#?(原神|星铁)?(开启|关闭)体力推送\s*/i, "").trim()
    );
    ...
    if (_.isEmpty(sks) || _.isEmpty(cks[g]))
      return e.reply("\n请【#扫码登录】后使用体力推送", false, { at: true });

    if (uid) {
      if (!cks[g][uid])
        return e.reply(
          `\n只能开启自己已绑ck及sk且未禁用的uid的体力提醒`,
          false,
          { at: true }
        );
      if (!sks[cks[g][uid].skid])
        return e.reply(
          `\n${game}UID:${uid} 请【#扫码登录】后使用体力推送`,
          false,
          { at: true }
        );
      ...
      for (let uid of uids[g]) {
        if (!sks[cks[g][uid].skid]) {
          e.reply(`\n${game}UID:${uid} 请【#扫码登录】后使用体力推送`, false, {
            at: true,
          });
          continue;
        }
      ...
      if (e.msg.includes("开启")) {
            ...
            [e.self_id]: {
              [e.user_id]: {
                [e.group_id]: {
                  gs: g === "gs" ? [uid] : [],
                  sr: g === "sr" ? [uid] : [],
                  gs_Resin: this.set.gs_Resin,
                  sr_Resin: this.set.sr_Resin,
                },
              },
            },
          };
        } else {
          if (!Notes[e.self_id]) Notes[e.self_id] = {};
          Notes[e.self_id][e.user_id] = Notes[e.self_id][e.user_id] || {};

          if (!Notes[e.self_id][e.user_id][e.group_id]) {
            Notes[e.self_id][e.user_id][e.group_id] = {
              gs: g === "gs" ? [uid] : [],
              sr: g === "sr" ? [uid] : [],
              gs_Resin: this.set.gs_Resin,
              sr_Resin: this.set.sr_Resin,
        ...
        if (Notes?.[e.self_id]?.[e.user_id]?.[e.group_id]) {
          ...
          if (uidIndex !== -1) {
            ...
            if (_.every(note, _.isEmpty))
              delete Notes[e.self_id][e.user_id][e.group_id];
            if (_.isEmpty(Notes[e.self_id][e.user_id]))
              delete Notes[e.self_id][e.user_id];
            if (_.isEmpty(Notes[e.self_id])) delete Notes[e.self_id];
          } else {
            return e.reply(`\n${game}UID:${uid}未在本群开启体力推送`, false, {
              at: true,
            });
          }
          Cfg.setConfig("defnote", Notes);
          return e.reply(`\n${game}UID:${uid}本群体力推送已关闭`, false, {
            at: true,
          });
        } else {
          return e.reply(`\n${game}UID:${uid}未在本群开启体力推送`, false, {
