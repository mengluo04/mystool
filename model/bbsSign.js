...
export default class BBsSign extends base {
    ...
    try {
      ...
      for (let forum of forumData) {
        ...
        if (res?.retcode == 1034) {
          ...
          message += `社区签到: ${
            res.retcode == 1008 ? "今日已签到" : `${res.message}`
          }\n`;
        }
        logger.mark(
          `${sk.id}:${forum.name} 社区签到结果: [${
            res.retcode == 1008 ? "今日已签到" : `${res.message}`
          }]`
        );
        ...
        while (
          !res.data?.list ||
          (res.data?.list?.length < time && Listretry < 2)
        ) {
  ...
  async bbsTask(manual) {
    ...
    logger.mark(
      `米币签到sk:${ltuids.length}个，预计需要${Cfg.countTime(
        time
      )} ${finishTime} 完成`
    );
  ...
  async result(res, userId) {
    ...
    if (res.retcode == -100) {
      this.invalidNum++;
      if (!this.invalidqq?.includes(userId)) this.invalidqq.push(userId);
    }
  ...
  async setCache(key) {
    ...
    if (!(await redis.get(key))) await redis.setEx(key, end, "1");
  }

  async getsignNum(ltuids) {
    let signNum = 0;
    for (let i of ltuids) if (await redis.get(`Bbs:Sign:${i}`)) signNum++;
