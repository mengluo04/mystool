...
export default class Note extends base {
  ...
  static async get(e) {
    ...
    if (_.every(cks, _.isEmpty))
      return e.reply("请【#扫码登录】绑定ck\n或尝试【#刷新ck】", false, {
        at: true,
      });

    e.reply("查询中请稍等...", false, { at: true });
    ...
    else await e._reply(sendMsg, false, { at: false });
          ...
          for (let g of ["gs", "sr"]) {
            ...
            for (let uid of Group[g]) {
              ...
              if (imgs[`${g}_${uid}`]) {
                ...
                logger.mark(
                  `[体力推送]Bot:${bot_id};Group:${group_id};QQ:${user_id};${`${g}_UID`}:${uid}`
                );
              ...
              if (Data?.retcode !== 0 || _.isEmpty(User) || _.isEmpty(Sign))
                continue;

              Resins[`${g}_${uid}`] =
                Data?.data[`current_${this.e.isSr ? "stamina" : "resin"}`];
              if (Number(Resins[`${g}_${uid}`]) >= Number(Resin)) {
                ...
                data = this.e.isSr
                  ? await this.noteSr(Data, uid)
                  : await this.notegs(Data, uid);
                data = {
                  quality: 80,
                  ...this.screenData,
                  ...data,
                ...
                imgs[`${g}_${uid}`] = await puppeteer.screenshot(
                  this.e.isSr
                    ? `${data.srtempFile}dailyNote`
                    : `${data.gstempFile}dailyNote`,
                  data
                );
              }
  ...
  async getData(ck, game) {
    ...
    let data =
      game == "sr"
        ? await this.noteSr(res.Data, ck.uid)
        : await this.notegs(res.Data, ck.uid);
  ...
  async noteData(ck, game) {
    ...
    device_fp = await new MysInfo(this.e).checkCode(
      device_fp,
      "getFp",
      mysApi,
      {},
    ...
    Data = await new MysInfo(this.e).checkCode(
      Data,
      "dailyNote",
      mysApi,
      {},
    ...
    resUser = await new MysInfo(this.e).checkCode(
      resUser,
      "UserGame",
      mysApi,
      {},
    ...
    signInfo = await new MysInfo(this.e).checkCode(
      signInfo,
      "sign_info",
      mysApi,
      {},
  ...
  noteSr(res, uid) {
    ...
    let sricon = _.sample(
      fs
        .readdirSync(
          `${this._path}/plugins/mystool/resources/StarRail/img/role`
        )
  ...
  async notegs(res, uid) {
    ...
    for (let val of data.expeditions)
      if (String(val.status) === "Finished") val.percentage = 100;
      else val.percentage = 50;
    ...
    if (
      !data.home_coin_recovery_time &&
      data.current_home_coin < data.max_home_coin
    )
      data.home_coin_recovery_time =
        ((data.max_home_coin - data.current_home_coin) / 30) * 3600;

    if (data.home_coin_recovery_time > 0) {
      let coinDate = new Date(
        new Date().getTime() + data.home_coin_recovery_time * 1000
      );
      ...
      if (coinDay > 0) {
        ...
        let dayTime =
          (24 -
            ...
            moment(coinDate).diff(new Date(), "hours")) /
          24;
