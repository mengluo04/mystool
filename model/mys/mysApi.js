...
export default class MysApi {
  ...
  getUrl(type, data = {}) {
    let urlMap = new apiTool(this.uid, this.server, this.game).getUrlMap({
      ...data,
  ...
  async getData(type, data = {}, game = "", cached = false) {
    ...
    if (this.set.isLog)
      logger.error(
        `[米游社接口][${type}][${this.uid}] ${url} ${JSON.stringify(param)}`
      );
    ...
    if (typeof res === "string")
      if (res.startsWith("(")) res = JSON.parse(res.replace(/\(|\)/g, ""));
  ...
  cacheKey(type, data) {
    return (
      `Yz:${this.game}:mys:cache:` + md5(this.uid + type + JSON.stringify(data))
    );
  }
  ...
  async getAgent() {
    ...
    if (HttpsProxyAgent) return new HttpsProxyAgent(proxyAddress);
