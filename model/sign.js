...
export default class MysSign extends base {
  ...
  static async sign(e) {
    ...
    for (let g of mysSign.set.game) {
      ...
      for (let i = 0; i < uids[g].length; i++) {
        ...
        // 执行签到操作
        let res = await mysSign.doSign(ck, uid, g, name);

        // 如果 retcode 为 -1000，重试指定次数
        if (res.retcode === -1000) {
  ...
  async doSign(ck, uid, game, name) {
    ...
    if (
      (signInfo.retcode == -100 && signInfo.message == "尚未登录") ||
      (signInfo.retcode !== 0 && signInfo.message?.includes("请登录后重试"))
    ) {
      logger.error(`[${name}签到失败]${this.log} 绑定cookie已失效`);
      if (this.set.Autodelck) await Cfg.delck(ck.ltuid, ck.qq);
      return {
        retcode: -100,
        msg: `\n签到失败，绑定cookie已失效\n可【#刷新ck】`,
        is_invalid: true,
    ...
    if (res) {
      ...
      if (!this.signInfo.is_sign) totalSignDay++;

      let tips = "签到成功";

      if (this.signed) tips = "今日已签";
  ...
  async getReward(signDay) {
    ...
    if (reward && reward.length > 0) {
      ...
      if (reward.name && reward.cnt) reward = `${reward.name}*${reward.cnt}`;
    } else {
      reward = "";
    }
  ...
  async bbsSign(name, game) {
    ...
    if (!sign || this.signMsg == "Too Many Requests") {
      logger.mark(
        `[${name}签到失败]${this.log}：${sign.message || this.signMsg}`
      );
      return false;
    }
    ...
    if (sign.data?.gt) {
      ...
      try {
        if (res?.data?.validate) {
          ...
          if (sign.data?.gt) {
            logger.mark(
              `[${name}签到失败]${this.log}：${sign.message} 第${this.ckNum}个`
            );
            return false;
          } else {
            this.signMsg = "验证码成功";
            logger.mark(
              `[${name}签到成功]${this.log}:验证码成功 第${this.ckNum}个`
            );
            return true;
          }
        } else {
          logger.mark(
            `[${name}签到失败]${this.log}：${res.msg} 第${this.ckNum}个`
          );
          return false;
        }
      } catch (error) {
  ...
  async signTask(manual) {
    ...
    if (this?.e?.msg?.includes("重新")) (signing = false), (Nosign = 0);
    ...
    let length = 0;
    for (let game of this.set.game) length += uids[game].length;
    ...
    for (let i = 0; i < noSignNum; i++)
      time +=
        ...
        (Math.floor(Math.random() * 2000) + 4000) / 1000 +
        4;
    ...
    for (let game of this.set.game)
      smsg += `${game == "gs" ? "原神" : game == "sr" ? "星铁" : "绝区零"}：${
        uids[game].length
      }|`;
    tips.push(smsg);
    ...
    logger.mark(
      `签到ck:${length}个，预计需要${Cfg.countTime(time)} ${finishTime} 完成`
    );
    ...
    for (let g of this.set.game) {
      ...
      for (let i = 0; i < uids[g].length; i++) {
        ...
        logger.mark(`自动签到[${name}]·QQ:${ck.qq} UID:${ck.uid}`);
        ...
        if (ret.retcode === 0)
          ...
          invalidNum[g]++;
          if (!invalidqq?.includes(ck.qq)) invalidqq.push(ck.qq);
        } else {
          failNum[g]++;
        }

        if (this.signApi) this.signApi = false;
      }
    }
    ...
    for (let game of this.set.game)
      ...
      }：\n成功：${sucNum[game]} | 已签：${finshNum[game]} | 失败：${
        failNum[game]
      }`;
  ...
  async getsignNum(uids, length) {
    let signNum = 0;
    for (let g of this.set.game)
      for (let i of uids[g]) if (await redis.get(`${g}:Sign:${i}`)) signNum++;
